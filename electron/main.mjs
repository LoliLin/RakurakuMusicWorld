...
const FRONTEND_INDEX = () => {
  const candidate = resolveResource('frontend', 'index.html')
  return fs.existsSync(candidate) ? candidate : path.join(__dirname, '../frontend/index.html')
}
...
function createMainWindow() {
  ...
  const iconPath = resolveResource(process.platform === 'win32' ? 'icon.ico' : 'icon.png')
